...
function markUi(action, focus, payload = {}) {
    ...
    if (payload.selected_service_id !== undefined) {
        uiState.selected_service_id = payload.selected_service_id;
    }
...
function resolveServiceReference(service_id, service_query, fallbackId = null) {
    ...
    if (!q) {
        return {
            error: 'Provide service_id or service_query.'
        };
    }
    ...
    if (matches.length > 1) {
        return {
            error:
                ...
                matches.map(s => `${s.name} (${s.id})`).join(', ')
        };
    }
...
function resolveProductReference(product_id, product_query) {
    ...
    if (!q) {
        return {
            error: 'Provide product_id or product_query.'
        };
    }
...
function resolveTargetBooking(booking_id, customer_name) {
    ...
    if (matches.length === 0) {
        return {
            error: `No confirmed booking found for ${customer_name}.`
        };
    }
    ...
    server.registerTool(
        ...
        async ({
            ...
            if (
                ...
                return errorResult(
                    `Slot ${date} ${time} is no longer available. Choose another available slot.`
                );
            }
    ...
    server.registerTool(
        ...
        async ({
            ...
            if (
                ...
                return errorResult(
                    `Slot ${newDate} ${new_time} is already occupied. Choose another available slot.`
                );
            }
...
html,body{
  ...
  min-height:100%;
  background:
    radial-gradient(circle at 20% 0%,rgba(143,99,255,.16),transparent 36%),
    radial-gradient(circle at 100% 18%,rgba(255,79,157,.12),transparent 32%),
    var(--bg);
  color:var(--text);
...
.brand-mark{
  ...
  place-items:center;
  font-size:25px;
  background:linear-gradient(145deg,rgba(255,79,157,.28),rgba(143,99,255,.28));
  border:1px solid rgba(255,255,255,.12);
  box-shadow:0 10px 34px rgba(143,99,255,.18);
}

.brand h1{margin:0;font-size:26px;letter-spacing:.03em}
.brand p{margin:4px 0 0;color:var(--pink);font-size:12px;letter-spacing:.16em;text-transform:uppercase;font-weight:800}
...
.live-dot{
  ...
  box-shadow:0 0 15px rgba(112,242,140,.8);
}
...
.panel{
  background:linear-gradient(180deg,rgba(23,24,43,.95),rgba(14,16,30,.96));
  border:1px solid var(--line);
  border-radius:22px;
  padding:18px;
...
.date-chip.date-flash{
  color:#fff;
  background:linear-gradient(135deg,rgba(255,79,157,.42),rgba(143,99,255,.42));
...
.service.highlight{
  border-color:rgba(255,79,157,.98);
  background:linear-gradient(135deg,rgba(255,79,157,.16),rgba(143,99,255,.12));
  box-shadow:
...
.icon{
  ...
  place-items:center;
  font-size:21px;
  background:linear-gradient(145deg,rgba(255,79,157,.16),rgba(143,99,255,.16));
}

.name{font-weight:800;font-size:13px;line-height:1.25}
.meta{color:var(--muted);font-size:11px;margin-top:4px}
.price{color:#ff77b6;font-size:14px;font-weight:900;white-space:nowrap}
...
.slot.free{
  color:#a7ffb7;
  background:rgba(41,181,75,.15);
  border-color:rgba(112,242,140,.24);
}

.slot.busy{
  color:#ff9da5;
  background:rgba(209,52,65,.15);
  border-color:rgba(255,106,116,.30);
}
...
.avatar{
  ...
  place-items:center;
  font-size:24px;
  background:linear-gradient(145deg,rgba(143,99,255,.45),rgba(255,79,157,.25));
  border:1px solid rgba(255,255,255,.12);
}
...
.booking-service{font-size:18px;font-weight:900}
.booking-time{margin-top:7px;color:#d8dbee;font-size:13px}
...
.status.confirmed{
  color:#bcffc8;
  background:rgba(41,181,75,.16);
  border:1px solid rgba(112,242,140,.22);
}

.status.cancelled{
  color:#ffc0c5;
  background:rgba(209,52,65,.16);
  border:1px solid rgba(255,106,116,.24);
}
...
.stock{
  ...
  padding:4px 7px;
  border-radius:8px;
  background:rgba(41,181,75,.14);
}

.stock.out{
  color:#ffc0c5;
  background:rgba(209,52,65,.14);
}
...
@media(max-width:1120px){
  ...
  .products-panel .list{grid-template-columns:repeat(2,minmax(0,1fr))}
}
...
@media(max-width:760px){
  ...
  .products-panel .list{grid-template-columns:1fr}
  .slots{grid-template-columns:repeat(2,minmax(0,1fr))}
  .brand h1{font-size:20px}
  .live{font-size:11px;padding:9px 12px}
}
</style>
</head>
...
<div class="topbar">
  <div class="brand">
    ...
    <div>
      <h1>YETI NAIL STUDIO</h1>
      <p>Live salon system</p>
    </div>
  </div>
...
<section id="slotsPanel" class="panel">
  <div class="panel-title">
    <h2>СВОБОДНЫЕ СЛОТЫ</h2>
    <div id="displayDate" class="date-chip">—</div>
  </div>
  <div id="slots" class="slots"></div>
</section>

<section id="bookingPanel" class="panel">
  <div class="panel-title">
...
console.log(
    `Yeti Nail Studio MCP v1.5.0 running on port ${port}`
);
